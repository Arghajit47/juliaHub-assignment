...
test.describe.serial("API Tests with Comet chat", () => {
  // 1. GET - Validate status & response structure
  test("List all the users in comet chat", async ({ request }) => {
    let allResults = [];
    let page = 1;
    while (true) {
  ...
  test("DELETE all users from comet chat", async ({ request }) => {
    const resultsFilePath = path.join(
      process.cwd(),
      "assets",
      "comet-chat-users.json"
  ...
  test("List all the users in comet chat & verify that all users are reuploaded", async ({
    request,
  }) => {
    const results = path.join(process.cwd(), "assets", "comet-chat-users.json");
